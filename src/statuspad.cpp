...
void Statuspad::render() {
    ...
    if (x != -1 && y != -1) {
        ...
        gfx->write("char:", left + 13, top + 1);
        gfx->setCh(cpixel.ch, left + 18, top + 1);
        gfx->write("RGBA(fore):", left + 1, top + 2);
        gfx->write("(" + prefix('0', std::to_string(cpixel.r), 3) + "," + 
                        prefix('0', std::to_string(cpixel.g), 3) + "," + 
                        prefix('0', std::to_string(cpixel.b), 3) + "," +
                        prefix('0', std::to_string(cpixel.a), 3) + ")", left + 1, top + 3);
        gfx->setBackColor(cpixel.r, cpixel.g, cpixel.b, cpixel.a, left + width - 2, top + 3);
        gfx->write("RGBA(back):", left + 1, top + 4);
        gfx->write("(" + prefix('0', std::to_string(cpixel.br), 3) + "," + 
                        prefix('0', std::to_string(cpixel.bg), 3) + "," + 
                        prefix('0', std::to_string(cpixel.bb), 3) + "," +
                        prefix('0', std::to_string(cpixel.ba), 3) + ")", left + 1, top + 5);
        gfx->setBackColor(cpixel.br, cpixel.bg, cpixel.bb, cpixel.ba, left + width - 2, top + 5);
    } else {
        gfx->write("x:  -", left + 1, top + 1);
        gfx->write("y:  -", left + 7, top + 1);
        gfx->write("char: ", left + 13, top + 1);
        gfx->write("RGBA(fore):", left + 1, top + 2);
        gfx->write("(---,---,---,---)", left + 1, top + 3);
        gfx->write("RGBA(back):", left + 1, top + 4);
        gfx->write("(---,---,---,---)", left + 1, top + 5);
    }
}
